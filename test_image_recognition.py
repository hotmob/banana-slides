...
def parse_image_with_mineru(image_path: str):
    ...
    logger.info(f"开始使用 MinerU 解析图片: {image_path}")
    ...
    logger.info(f"MinerU API Base: {config.MINERU_API_BASE}")
    ...
    try:
        ...
        output_file = output_dir / f'mineru_parse_result_{Path(image_path).stem}.md'
...
if __name__ == '__main__':
    ...
    print("=" * 80)
    print("MinerU 图片解析测试脚本")
    print("=" * 80)
    print()
    
    markdown_content, extract_id, output_file = parse_image_with_mineru(image_path)
